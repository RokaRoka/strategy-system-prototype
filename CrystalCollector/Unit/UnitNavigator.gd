...
#signals to emit to main unit script
signal job_done
...
func _draw():
	if navMeshInstance:
		#draw debug lines
		draw_polyline( navMeshInstance.navpoly.get_vertices(), debugColor, 10.0)
...
func walk(var speed):
	#print("Global Position: "+String(global_position))
	#print("Parent Position: "+String(get_parent().global_position))
	#calculate direction
...
func findPath(var newPosition):
	...
	newPos = get_closest_point(newPosition)
	oldPos = get_parent().global_position
